...
let ItemController = {
  ...
  addItem: async (req: Request, res: Response, next: NextFunction) => {
    ...
    const totalBatches = Math.max(
      Math.round(
        ((req.files as Express.Multer.File[]).length as number) /
          desiredBatchSize
      ),
      1
    );
    ...
    for (var i = 0; i < (req.files as Express.Multer.File[]).length; i++) {
      // creates the new labelled item json object
      newLabelledItemObject = {
        originalname: (req.files as Express.Multer.File[])[i].originalname,
